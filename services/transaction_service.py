"""BridgeEscrow — Transaction CRUD and status advancement."""
...
def _next_txn_id():
    ...
    n = max((int(t["txn_id"][3:]) for t in txns
             if t["txn_id"].startswith("TXN") and t["txn_id"][3:].isdigit()), default=0)
...
def create_transaction(product_id, buyer):
    ...
    if product.stock <= 0: print("\n✗ Out of stock."); return None
    if buyer.balance < product.price: print("\n✗ Insufficient balance."); return None
    txn_id = _next_txn_id()
    txn = Transaction(txn_id, product_id, buyer.user_id, product.retailer_id, product.price)
    buyer.balance -= product.price; buyer.locked += product.price
    product.stock -= 1; update_user(buyer); save_product(product)
    txns = db.load("transactions"); txns.append(txn.to_dict()); db.save("transactions", txns)
    add_ledger_entry(txn_id, "HOLD", product.price, from_id=buyer.user_id)
    print(f"\n✓ Transaction {txn_id} created. KSh {product.price:,} held."); return txn
...
def advance_transaction(txn_id, user, code=None):
    ...
    if nxt not in config.ROLE_PERMISSIONS.get(user.role, []):
        print(f"\n✗ Cannot advance to {nxt}."); return None
    if nxt == "REFUNDED" and txn.status in config.NO_REFUND_STATUSES:
        print("\n✗ Cannot refund — funds already released."); return None
    if nxt == "PAID": txn.delivery_code = generate_delivery_code()
    if nxt == "DELIVERED":
        if not verify_delivery_code(txn, code):
            print("\n✗ Invalid delivery code. Get it from the buyer."); return None
        txn.delivery_id = user.user_id
    txn.status = nxt; _save_transaction(txn); return _post_advance(txn, nxt, txn_id)


def _post_advance(txn, nxt, txn_id):
    if nxt == "DELIVERED":
        txn.status = "RELEASED"; _save_transaction(txn); release_funds(txn)
        print(f"\n✓ {txn_id} → DELIVERED → RELEASED (auto)"); return txn
    if nxt == "PAID":
        print(f"\n✓ {txn_id} → PAID | Code: {txn.delivery_code} — share with delivery person!"); return txn
    if nxt == "REFUNDED": refund_funds(txn)
    label = nxt.replace("SHIPPED", "READY FOR DELIVERY")
    print(f"\n✓ Transaction {txn_id} → {label}"); return txn


def find_transaction(txn_id):
    for t in db.load("transactions"):
        if t["txn_id"] == txn_id: return Transaction.from_dict(t)
    return None


def list_transactions_by_user(user_id, role=None):
    txns = db.load("transactions")
    owned = {t["txn_id"] for t in txns
             if t["buyer_id"] == user_id or t["retailer_id"] == user_id
...
def _save_transaction(txn):
    ...
    for i, t in enumerate(txns):
        if t["txn_id"] == txn.txn_id: txns[i] = txn.to_dict(); break
