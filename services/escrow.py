"""BridgeEscrow — Escrow fund release and refund logic."""
...
def release_funds(txn):
    """Release held escrow: retailer gets goods value minus fees,
    delivery agent gets delivery fee, trustee gets commission."""
...
def refund_funds(txn):
    """Refund held escrow funds back to the buyer."""
    buyer = find_user_by_id(txn.buyer_id)
    ...
    add_ledger_entry(txn.txn_id, "REFUND", txn.amount,
                     to_id=buyer.user_id)